...
showNeueBestellung({
  ...
  required List<Kassen> KassenList,
...
class __BuildSheetProState extends State<_BuildSheetPro> {
  Shop? selectedVal;
  Kassen? selectedKasse;
  List<Kassen> ?filterdKassenList;
  ...
  void initState() {
    ...
    SchedulerBinding.instance?.addPostFrameCallback((_) {
      setState(() {
       ...
       filterdKassenList = widget.KassenList.where((element) => element.shopId == selectedVal?.id).toList();
        selectedKasse =filterdKassenList![0];
      });
    });
  }
        ...
        children: [
          ...
          Padding(
            ...
            child: DropdownButton(
                ...
                onChanged: (Shop? newVal) {
                  ...
                  setState(() {
                    selectedVal = newVal;
                    filterdKassenList = widget.KassenList.where((element)
                    => element.shopId == selectedVal?.id).toList();
                    selectedKasse = filterdKassenList![0];
          ...
          Padding(
            ...
            child: DropdownButton(
                value: selectedKasse,
                isExpanded: true,
                disabledHint: Text('Select Shop'),
                hint: Text('Select Shop'),
                icon: const Icon(Icons.keyboard_arrow_down),
                items: filterdKassenList?.map((singleKassen) {
                  return DropdownMenuItem<Kassen>(
                    value: singleKassen,
                    enabled: true,
                    alignment: AlignmentDirectional.centerStart,
                    child: Text(singleKassen.kasssName.toString()),
                  );
                }).toList(),
                onChanged: (Kassen? newVal) {
                  widget.onChangeValue(newVal);
                  setState(() {
                    selectedKasse = newVal;
                  });
                }),
          ),
          const SizedBox(
